...
INSERT INTO `receiving_list` (`id`, `from_id`, `from_order`, `amount`, `discount_perc`, `discount`, `tax_perc`, `tax`, `stock_ids`, `remarks`, `date_created`, `date_updated`) VALUES
...
/* trigger to update the date_updated field whenever there is an update on the purchase_order_list table*/
DELIMITER //

CREATE TRIGGER update_purchase_order_list_date
BEFORE UPDATE ON purchase_order_list
FOR EACH ROW
BEGIN
...
/*
Add one function that returns tax_perc, tax, discount, discount price
Remove those attributes from backorder list and receiving list
Make report
*/

'''
/*a stored procedure that retrieves the total amount of a back order and its items*/
DELIMITER //

CREATE PROCEDURE GetBackOrderTotal(IN boID INT)
BEGIN
    DECLARE totalAmount FLOAT;
    DECLARE totalItems FLOAT;

    SELECT SUM(amount) INTO totalAmount FROM back_order_list WHERE id = boID;
    SELECT SUM(total) INTO totalItems FROM bo_items WHERE bo_id = boID;

    SELECT totalAmount + totalItems AS total;
END;
...
/*modify the stored procedure to include a cursor*/

DELIMITER //

CREATE PROCEDURE GetBackOrderDetails(IN boID INT)
BEGIN
    DECLARE totalAmount FLOAT;
    DECLARE totalItems FLOAT;
    DECLARE item_id INT;
    ...
    DECLARE cur_bo_items CURSOR FOR
        SELECT item_id, quantity, price, unit, total
        FROM bo_items
        WHERE bo_id = boID;
    ...
    -- Calculate the total amount from the back order
    SELECT SUM(amount) INTO totalAmount FROM back_order_list WHERE id = boID;

    -- Iterate through the cursor
    OPEN cur_bo_items;
    read_loop: LOOP
        FETCH cur_bo_items INTO item_id, item_quantity, item_price, item_unit, item_total;
        IF done THEN
            LEAVE read_loop;
        END IF;

        -- Do additional processing with cursor values if needed
        -- For example, you can display or aggregate data here

        -- Accumulate the total cost and quantity
        SET totalCost = totalCost + item_total;
        SET totalQuantity = totalQuantity + item_quantity;
    END LOOP;
    CLOSE cur_bo_items;

    -- Return the total amount, total cost, and total quantity
    SELECT totalAmount AS back_order_amount, totalCost AS total_item_cost, totalQuantity AS total_item_quantity;
END;
...
/*function that calculates the total cost of items in a purchase order.*/
DELIMITER //

CREATE FUNCTION CalculatePOTotalCost(poID INT)
RETURNS FLOAT
DETERMINISTIC
BEGIN
    DECLARE totalCost FLOAT;

    SELECT SUM(total) INTO totalCost FROM po_items WHERE po_id = poID;
...
/*retrieve information about back orders, their items, and related suppliers.*/
-- Example 1: Retrieve back order information with items and supplier details
SELECT bo.*, bi.*, sl.name AS supplier_name
FROM back_order_list bo
JOIN bo_items bi ON bo.id = bi.bo_id
JOIN supplier_list sl ON bo.supplier_id = sl.id
WHERE bo.id = 1;

-- Example 2: Retrieve purchase order information with items and supplier details
SELECT po.*, poi.*, sl.name AS supplier_name
FROM purchase_order_list po
JOIN po_items poi ON po.id = poi.po_id
JOIN supplier_list sl ON po.supplier_id = sl.id
WHERE po.id = 1;
